...
public class IdWorker {
	...
	//定义一个开始纪元, 用(当前时间-开始纪元）算出time 可以直接根据time进行排序
	...
	private long sequenceBits = 10L; // 毫秒内自增位
	...
	public IdWorker(long workerId, long datacenterId) {
		...
		/*System.out.println(String.format(
				"worker starting. timestamp left shift %d, datacenter id bits %d, "
	...
	public synchronized long nextId() {
		...
		if (timestamp < lastTimestamp) {
			/*System.out.println(String.format(
					...
					lastTimestamp));*/
			log.error("timestamp = {}, lastTimestamp = {}", DateFormatUtils.format(timestamp, DateUtils.YYYY_MM_DD_HH_MM_SS), DateFormatUtils.format(lastTimestamp, DateUtils.YYYY_MM_DD_HH_MM_SS));
